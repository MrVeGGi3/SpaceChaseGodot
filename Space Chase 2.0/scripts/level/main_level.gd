...
@export var hungry_maximum_time_ : int
@export var thirsty_maximum_time : int
@export var crazyness_maximum_time : int
@export var sleepy_maximum_time : int
@export var oxygen_maximum_time : int

@export_category("Oxygen inside/outside control")
@export var loss_oxygen_external : int
@export var loss_oxygen_internal : int

@export_category("Loss of Status per Minute")
@export var loss_hungry_per_min : int
@export var loss_thirsty_per_min : int
@export var loss_crazyness_per_min : int
@export var loss_sleepy_per_min : int
...
var oxygen_limit_counter : float
...
func _process(delta: float) -> void:
	...
	if GameManager.get_player_outside():
		player.oxygen_tank -= loss_oxygen_internal
	else:
		player.oxygen_tank -= loss_oxygen_internal
	...
	if player._get_oxygen_limit():
		oxygen_limit_counter += delta
		if oxygen_limit_counter >= oxygen_maximum_time:
			GameOver()
	else:
		oxygen_limit_counter = 0.0
